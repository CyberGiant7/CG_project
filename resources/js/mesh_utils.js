...
function find_in_vert_face(vert, ind_vert, ind_face) {
  var k, nval;
  if (vert[ind_vert].bound) nval = vert[ind_vert].val - 1;
  else nval = vert[ind_vert].val;
  for (k = 0; k < nval; k++) if (vert[ind_vert].face[k] == ind_face) break;
  return k;
}
...
function find_in_face(face, ind_face, ind_vert) {
  var k;
  for (k = 0; k < face[ind_face].n_v_e; k++) if (face[ind_face].vert[k] == ind_vert) break;
  return k;
}
...
function convert_obj2grid_data(model, gc_grid) {
  ...
  for (var h = 0; h < grid.nnodes * 3; h++) grid.nodes.push(new Array(3));
  ...
  for (i = 0; i < model.numtriangles; i++) nvf += model.triangles[i].nv;
...
function convert_grid_data2mesh(grid, mesh) {
  ...
  for (var h = 0; h < mesh.nvert + 1; h++) mesh.vert[h] = new nvr();

  mesh.face = new Array(mesh.nface + 2);
  for (var h = 0; h < mesh.nface + 2; h++) mesh.face[h] = new nfc();
...
function LoadSubdivMesh(mesh) {
  ...
  for (k = 1; k <= nface; k++) {
    for (ii = 0; ii < mesh.face[k].n_v_e; ii++) {
      ...
      kk = buf[i].n;
      j = 0;
      while (j < kk && buf[i].vedge[j] != mesh.face[k].vert[i_st]) j++;
      ...
      if (j == kk) {
        ...
        kk = buf[i].n;
        j = 0;
        while (j < kk && buf[i].vedge[j] != mesh.face[k].vert[i_en]) j++;
  ...
  if (edge == null) console.log("NON sono riuscito ad allocare memoria\n");
  ...
  for (k = 1; k <= nface; k++) {
    for (ii = 0; ii < mesh.face[k].n_v_e; ii++) {
      ...
      if (buf[kk].vedge[count_fe[kk]] == mesh.face[k].vert[(ii + 1) % mesh.face[k].n_v_e]) {
        ...
        while (i < buf[kk].n && buf[kk].vedge[i] != mesh.face[k].vert[ii]) i++;
  ...
  for (k = 1; k <= nvert; k++) {
    for (i = 0; i < buf[k].n; i++) {
      ...
      } else mesh.edge[buf[k].epos[i]].neigh[1] = 0;
  ...
  for (i = 1; i <= nvert; i++) {
    ...
    if (mesh.vert[i].bound)
      for (k = 0; k < mesh.vert[i].val - 1; k++) {
        ...
        if (k == 0) ii = find_in_face(mesh.face, jj, i);
        else ii = find_in_face(mesh.face, jj, mesh.vert[i].neigh[ic - 1]);
  ...
  for (i = 1; i <= nface; i++) {
    ...
    while (j < mesh.face[i].n_v_e && !mesh.face[i].bound) {
      if (mesh.vert[mesh.face[i].vert[j]].bound) mesh.face[i].bound = true;
      j++;
    }
  }
  ...
  for (i = 1; i <= nface; i++) {
    ...
    if (mesh.face[i].bound) {
      ...
      ij = 0;
      while (!mesh.vert[mesh.face[i].vert[ij]].bound) ij++;
    ...
    for (j = 0; j < mesh.face[i].n_v_e; j++) {
      ...
      if (mesh.vert[jj].bound) nval = mesh.vert[jj].val - 1;
      else nval = mesh.vert[jj].val;
    ...
    if (ic > 1 && mesh.face[i].neigh[0] == mesh.face[i].neigh[ic - 1]) ic--;
